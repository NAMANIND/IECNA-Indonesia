  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.registrationType === "nomination") {
      ...
      const imageRef = storage
        .ref()
        .child(`indonesia-nomination-image/${formData.image.name}`);
      ...
      const htmlcontent = `
        ...
        <p>Vote Link: ${`
      https://indonesia.theiena.com/vote/${formData.firstName
        .toLowerCase()
      ...
      const vlink = `https://indonesia.theiena.com/vote/${formData.firstName
        .toLowerCase()
        .replace(/\s/g, "")}_${formData.lastName
        .toLowerCase()
        .replace(/\s/g, "")}`;
      ...
      const nomineeRef = firestore.collection("indonesia-nominees").doc();
      const nomineeId = nomineeRef.id;
      const nomineeQuery = firestore
        .collection("indonesia-nominees")
        .where(
          "firstName",
          "==",
          formData.firstName.toLowerCase().replace(/\s/g, "")
      ...
      const imageRef = storage
        .ref()
        .child(`indonesia-delegate-image/${formData.image.name}`);
      await imageRef.put(formData.image);
    ...
    `;
      ...
      const nomineeRef = firestore.collection("indonesia-delegates").doc();
      const nomineeId = nomineeRef.id;
