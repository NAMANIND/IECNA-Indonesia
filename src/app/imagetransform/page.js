...
function ImageDownloadPage({
  ...
  useEffect(() => {
    const uploadImage = async () => {
      ...
      formData.append("upload_preset", "iecna_upload_indonesia");
  ...
  const applyTransformations = (publicId) => {
    ...
    if (category === "speaker") {
      if (field === "marketer") {
        const transformedImageUrl =
          ...
          `l_iecna_upload_indonesia:${overlayParam}/fl_layer_apply,ar_1.0,c_lfill,w_209,h_209,x_315,y_-34,r_max/c_scale/co_rgb:FFFFFF,l_text:arial_24_bold_normal_left:${encodeURIComponent(
            title
    ...
    if (category == "delegate") {
      if (field === "marketer") {
        const transformedImageUrl =
          `https://res.cloudinary.com/dmedpnbvc/image/upload/` +
          `l_iecna_upload_indonesia:${overlayParam}/fl_layer_apply,ar_1.0,c_lfill,w_338,h_338,x_280,y_-69,r_max/c_scale/co_rgb:FFFFFF,l_text:arial_34_bold_normal_left:${encodeURIComponent(
            title
    ...
    if (category === "nomination") {
      if (field === "marketer") {
        const transformedImageUrl =
          `https://res.cloudinary.com/dmedpnbvc/image/upload/` +
          `l_iecna_upload_indonesia:${overlayParam}/fl_layer_apply,ar_1.0,c_lfill,w_360,h_360,x_-6,y_52,r_max/c_scale/co_rgb:FFFFFF,l_text:arial_34_bold_normal_left:${encodeURIComponent(
            title
  ...
  const handleImageLoad = () => {
    ...
    if (transformedImageUrl !== null) {
      const trfRef = firestore.collection("indonesia-transformed-images");
      trfRef
        .where("trf", "==", rem)
        .get()
        .then((querySnapshot) => {
